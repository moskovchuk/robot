...
class Parser(object):
    ...
    tokens = Lexer.tokens
    precedence = (
        ('nonassoc', 'IFLESS', 'IFNLESS', 'IFZERO', 'IFNZERO', 'IFHIGH', 'IFNHIGH'),
        ('left', 'PLUS'),
        ('right', 'ASSIGNMENT', 'MINUS')
    )
    ...
    def p_statements(self, p):
        ...
        if len(p) == 2:
            ...
            p[0] = SyntaxTreeNode('statements', children=[p[1], SyntaxTreeNode('statement', children=p[2])])
    ...
    def p_declaration(self, p):
        ...
        if len(p) == 3:
            p[0] = SyntaxTreeNode('declaration', value='VARIANT', children=p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
        else:
            p[0] = SyntaxTreeNode('declaration', value='VARIANT', children=[SyntaxTreeNode('init', value=p[2], lineno=p.lineno(1), lexpos=p.lexpos(1)), p[4],
                                                                            SyntaxTreeNode('init_end')], lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_variant(self, p):
        ...
        if len(p) == 2 and p[1] == 'PARAM':
            p[0] = SyntaxTreeNode('func_param', value=p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
        elif p[1] == 'PARAM':
            p[0] = SyntaxTreeNode('func_param', value=p[1], children=p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
        elif len(p) == 2:
            p[0] = SyntaxTreeNode('variant', value=p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
        else:
            p[0] = SyntaxTreeNode('variant', value=p[1], children=p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_varsize(self, p):
        ...
        if len(p) == 4:
            p[0] = SyntaxTreeNode('varsize', children=p[2], lineno=p.lineno(2), lexpos=p.lexpos(2))
        elif len(p) == 3:
            p[0] = SyntaxTreeNode('empty_varsize', lineno=p.lineno(1), lexpos=p.lexpos(1))
        else:
            p[0] = SyntaxTreeNode('varsize', children=[p[2], p[4]], lineno=p.lineno(2), lexpos=p.lexpos(2))

    def p_initialization(self, p):
        """initialization : OFBRACKET init_lists CFBRACKET"""
        p[0] = p[2]
    ...
    def p_init_lists(self, p):
        ...
        if len(p) == 2:
            ...
            p[0] = SyntaxTreeNode('init_lists', children=[p[1], p[2]])
    ...
    def p_init_list(self, p):
        ...
        if len(p) == 4:
            ...
            p[0] = SyntaxTreeNode('empty_init_list', lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_const_expression(self, p):
        """const_expression : const_math_expression
                        | const"""
        p[0] = SyntaxTreeNode('const_expression', children=p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_decimal_expression(self, p):
        """decimal_expression : dec_math_expression
                                ...
                                | variant
                                | function_call"""
        p[0] = SyntaxTreeNode('decimal_expression', children=p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_bool_expression(self, p):
        """bool_expression : bool_math_expression
                            ...
                            | variant
                            | function_call"""
        p[0] = SyntaxTreeNode('bool_expression', children=p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_string_expression(self, p):
        """string_expression : string_math_expression
                            ...
                            | variant
                            | function_call"""
        p[0] = SyntaxTreeNode('string_expression', children=p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_math_expression(self, p):
        """math_expression : expression PLUS expression
                            | MINUS expression"""
        if len(p) == 3:
            p[0] = SyntaxTreeNode('unar_op', value=p[1], children=p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
        else:
            p[0] = SyntaxTreeNode('bin_op', value=p[2], children=[p[1], p[3]], lineno=p.lineno(2), lexpos=p.lexpos(2))
    ...
    def p_decimal_const(self, p):
        ...
        p[0] = SyntaxTreeNode('decimal_const', value=p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_bool_const(self, p):
        ...
        if p[1] == 'TRUE':
            ...
            val = False
        p[0] = SyntaxTreeNode('bool_const', value=val, lineno=p.lineno(1), lexpos=p.lexpos(1))

    def p_string_const(self, p):
        """string_const : LETTERS"""
        p[0] = SyntaxTreeNode('string_const', value=p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))

    def p_assignment(self, p):
        """assignment : variant ASSIGNMENT expression"""
        p[0] = SyntaxTreeNode('assignment', value=p[1], children=p[3], lineno=p.lineno(2), lexpos=p.lexpos(2))

    def p_convert(self, p):
        """convert : CONVERT type TO type variant"""
        p[0] = SyntaxTreeNode('convert', value=[p[2], p[4]], children=p[5], lineno=p.lineno(1), lexpos=p.lexpos(1))

    def p_digitize(self, p):
        """digitize : DIGITIZE type variant"""
        p[0] = SyntaxTreeNode('digitize', value=p[2], children=p[3], lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_while(self, p):
        ...
        p[0] = SyntaxTreeNode('while', children={'condition': p[2], 'body': p[4]}, lineno=p.lineno(1), lexpos=p.lexpos(1))

    def p_until(self, p):
        """until : UNTIL bool_expression NEWLINE statements ENDU
                | UNTIL bool_expression NEWLINE func_body_statements ENDU"""
        p[0] = SyntaxTreeNode('until', children={'condition': p[2], 'body': p[4]}, lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_if(self, p):
        ...
        if len(p) == 6:
            p[0] = SyntaxTreeNode('if', children={'condition': SyntaxTreeNode('condition', value=p[1], lineno=p.lineno(1), lexpos=p.lexpos(1)),
                                        'conditional_expressions': SyntaxTreeNode('conditional_expressions', children=p[2], lineno=p.lineno(2), lexpos=p.lexpos(2)),
                                            'body': p[4]}, lineno=p.lineno(1), lexpos=p.lexpos(1))
        else:
            p[0] = SyntaxTreeNode('if', children={'condition': SyntaxTreeNode('condition', value=p[1], lineno=p.lineno(1), lexpos=p.lexpos(1)),
                                    'conditional_expressions': SyntaxTreeNode('conditional_expressions', children=[p[2], p[4]], lineno=p.lineno(2), lexpos=p.lexpos(2)),
                                            'body': p[6]}, lineno=p.lineno(1), lexpos=p.lexpos(1))

    def p_command(self, p):
        """command : COMMAND string_expression"""
        p[0] = SyntaxTreeNode('command', children=p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_func_body_statements(self, p):
        ...
        if len(p) == 2:
            ...
            p[0] = SyntaxTreeNode('func_body_statements', children=[p[1], SyntaxTreeNode('func_body_statement', children=p[2])])
    ...
    def p_function(self, p):
        ...
        self._functions[p[2]] = SyntaxTreeNode('function', children={'body': p[4]}, lineno=p.lineno(1), lexpos=p.lexpos(1))
        p[0] = SyntaxTreeNode('func_descriptor', value=p[2], lineno=p.lineno(1), lexpos=p.lexpos(2))

    def p_function_call(self, p):
        """function_call : CALL NAME expression
                            | CALL NAME"""
        if len(p) == 4:
            p[0] = SyntaxTreeNode('function_call', value={'name': p[2]},children=p[3], lineno=p.lineno(1), lexpos=p.lexpos(1))
        else:
            p[0] = SyntaxTreeNode('function_call', value={'name': p[2]}, lineno=p.lineno(1), lexpos=p.lexpos(1))

    def p_return(self, p):
        """return : RETURN expression"""
        p[0] = SyntaxTreeNode('return', children=p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_decl_error(self, p):
        """declaration : VARIANT error
                        ...
                        | declaration error"""
        if len(p) == 3:
            p[0] = SyntaxTreeNode('error', value='Declaration error', lineno=p.lineno(2), lexpos=p.lexpos(2))
            sys.stderr.write(f'==> Error in variant declaration\n')
        else:
            p[0] = SyntaxTreeNode('error', value='Initialization error', lineno=p.lineno(2), lexpos=p.lexpos(2))
    ...
    def p_varsize_error(self, p):
        """varsize : OSQBRACKET error CSQBRACKET
                    ...
                    | varsize error"""
        p[0] = SyntaxTreeNode('error', value='Variant size/index error', lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_assignment_error(self, p):
        ...
        p[0] = SyntaxTreeNode('error', value='Assignment error', children=p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_convert_error(self, p):
        """convert : CONVERT type TO type error
                    ...
                    | CONVERT error
                    | convert error"""
        p[0] = SyntaxTreeNode('error', value='Convertation error', lineno=p.lineno(1), lexpos=p.lexpos(1))
        sys.stderr.write(f'==> Error in convertation!\n')

    def p_digitize_error(self, p):
        """digitize : DIGITIZE error
                    | digitize error"""
        p[0] = SyntaxTreeNode('error', value='Digitize error', lineno=p.lineno(1), lexpos=p.lexpos(1))
        sys.stderr.write(f'==> Error in digitize!\n')

    def p_command_error(self, p):
        """command : COMMAND error
                    | command error"""
        p[0] = SyntaxTreeNode('error', value='Command error', lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_while_error(self, p):
        """while : WHILE error NEWLINE statements ENDW
                    ...
                    | while error"""
        if len(p) == 6:
            p[0] = SyntaxTreeNode('while_error', value='While error', children={'body': p[4]}, lineno=p.lineno(1), lexpos=p.lexpos(1))
            sys.stderr.write(f'==> Error in \'while\'!\n')
        elif len(p) == 5:
            p[0] = SyntaxTreeNode('while_error', value='While error', children={'body': p[3]}, lineno=p.lineno(1), lexpos=p.lexpos(1))
            try:
            ...
            p[0] = SyntaxTreeNode('while_error', value='While error', lineno=p.lineno(1), lexpos=p.lexpos(1))
            sys.stderr.write(f'==> Error in \'while\'!\n')
    ...
    def p_until_error(self, p):
        """until : UNTIL error NEWLINE statements ENDU
                    ...
                    | until error"""
        if len(p) == 6:
            p[0] = SyntaxTreeNode('until_error', value='Until error', children={'body': p[4]}, lineno=p.lineno(1), lexpos=p.lexpos(1))
            sys.stderr.write(f'==> Error in \'until\'!\n')
        elif len(p) == 5:
            p[0] = SyntaxTreeNode('until_error', value='Until error', children={'body': p[3]}, lineno=p.lineno(1), lexpos=p.lexpos(1))
            try:
                sys.stderr.write(f'Error at {p.lineno(1)} line\n')
            except:
                sys.stderr.write(f'Error\n')
            ...
            p[0] = SyntaxTreeNode('until_error', value='While error', lineno=p.lineno(1), lexpos=p.lexpos(1))
            sys.stderr.write(f'==> Error in \'until\'!\n')
    ...
    def p_if_error(self, p):
        """if : IFLESS error NEWLINE statements ENDIF
              ...
              | if error"""
        p[0] = SyntaxTreeNode('if_error', value='\'If\' error', lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    def p_function_error(self, p):
        """function : FUNC error NEWLINE func_body_statements ENDFUNC
                    ...
                    | function error"""
        if len(p) == 6:
            self._functions[p[2]] = SyntaxTreeNode('function_error', children={'body': p[4]}, lineno=p.lineno(1), lexpos=p.lexpos(1))
            p[0] = SyntaxTreeNode('func_error', value='Function error!', lineno=p.lineno(1), lexpos=p.lexpos(2))
    ...
    def p_return_error(self, p):
        """return : RETURN error
                    | return error"""
        p[0] = SyntaxTreeNode('return_error', value='Return error', lineno=p.lineno(1), lexpos=p.lexpos(1))
